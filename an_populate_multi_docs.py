...
def generate_map_ds(company_id, doc_tup):
    ...
    if 1:#try:
        ...
        if 1: 
         for key, value_dict in celldata.items():
            ...
            for cell_id in cell_ids:
                ...
                all_txts = []
                for txt in cell_info.get('text_lst', []):
                    new_txt = []
                    if ('.' in txt or ',' in txt) and section_type == 'GV':
                        #print 'before',[ txt]
                        for e in txt:
                            if e == '.':
                                e = ','
        ...
        html_str = display_table(data_ar, norm_table_id, doc_id) 
        #############################################################################
        new_data_ar = normalise_colspan(data_ar)
        normalised_hyp_data = normalise_rowspan(new_data_ar)
        #######################################################
        hyp_line_item_map, hyp_value_to_line_item_map, norm_row_col_map_dict, line_item_map_ref = form_search_data_structure(normalised_hyp_data, norm_table_id, doc_id)
        row_col_lmdb_path = os.path.join(output_path, company_id, 'row_col_map_data')
        if not os.path.exists(row_col_lmdb_path):
            cmd = "mkdir -p %s" %(row_col_lmdb_path)
            os.system(cmd)
        row_col_fname = os.path.join(row_col_lmdb_path, norm_table_id) 
        lmdb_obj.write_to_lmdb(row_col_fname, norm_row_col_map_dict, norm_row_col_map_dict.keys())
        ########################################################
        lmdb_folder = os.path.join(output_path, company_id) 
        if not os.path.exists(lmdb_folder):
            cmd = "mkdir -p %s" %(lmdb_folder)
            os.system(cmd)
        ...
        table_fname = os.path.join(lmdb_fname, norm_table_id)
        if not os.path.exists(table_fname):
            os.mkdir(table_fname)
        
        allkey_dict = {}
        allkey_dict[norm_table_id] = (hyp_line_item_map, hyp_value_to_line_item_map, value_table_id_map, line_item_map_ref)
        lmdb_obj.write_to_lmdb(table_fname, allkey_dict, allkey_dict.keys())
...
def display_table(data_ar, norm_table_id, docid):
    page_no = ''
    if data_ar:
    ...
    html_str += 'TABLE ID: '+ str(norm_table_id) + '   Document ID: '+ str(docid) +'  PAGE NO: ' + str(page_no)
    html_str += '</div>'
