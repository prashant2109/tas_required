...
def insert_update_table_report(company_id, table_ids):
    ...
    tids = [] 
    data = []
    for (doc_id, table_id, page_no, g_id, lng) in table_ids:
...
def generate(company_id, given_doc_ids=None, lang=''):
    ...
    for doc_tup in norm_res_list:
        ...
        ktup = (doc_id, norm_table_id, page_number, gr_id, lang)
...
def generate_map_ds(company_id, doc_tup):
    ...
    project_id, url_id = company_id.split('_')
    doc_id, norm_table_id, page_no, gr_id, lang = doc_tup
    #if norm_table_id != '551':
    #    return [doc_id, norm_table_id, '', 0, 'ME', '']
    deal_id = '_'.join([project_id, url_id])
    print 'DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD', doc_tup
    #try:
    ...
    if 1:#try:
        ...
        iusr, nusr = 'test_user', 'dev'#= get_analyst_name(company_id, norm_table_id)
        print '<<<<<<<<<', doc_tup
