...
class Company_populate_new(object):
    ...
    def prepare_metadata_rows(self, doc_ph):
        data_rows = []
        for doc_id, dt_tup in doc_ph.iteritems():
            #doc_name = '%s.pdf'%(doc_id)
            data_tup = ('', '', '', dt_tup[0], dt_tup[1], dt_tup[2], '', '', '', '', '', '', '0', doc_id, '', '')
            data_rows.append(data_tup)
        return data_rows

    def add_meta_data_hra(self, comp, doc_date_lst, cid, project_id): 
        get_doc_ph_dct, docids  =  self.get_doc_ph_dct_hra(doc_date_lst)
        data_rows = self.prepare_metadata_rows(get_doc_ph_dct)
    ...
    def working_with_d2_table_dict_data_doc_table_wise(self, company_name, model_number, company_id, html_cell_data_dict, dc_ph_dct, s_type):
        project_id, deal_id = company_id.split('_')
        ...
        for doc_page_tup, html_cell_dct in html_cell_data_dict.iteritems():
            ...
            if (source_doc_info, source_type) not in from_doc_map:
                ...
                #ins_stmt = """INSERT INTO ir_document_master(document_id, project_id, url_id, document_name, active_status, source_doc_info, source_type) values('%s', '%s', '%s', '%s', '%s', '%s', '%s')"""%(d, project_id, deal_id, document_name, 'Y', source_doc_info, source_type) 
                i_doc_ar.append((new_docid, project_id, deal_id, d_n, 'Y', source_doc_info, source_type))
            ...
            if (source_table_info, source_type)  not in from_table_map:
                ...
                max_table   += 1
                i_table_ar.append((new_table, project_id, deal_id, doc_id, new_table, page_no,  'Y', 'Y', 'Y', source_table_info, source_type))
                i_table_ar2.append((new_table, project_id, deal_id, doc_id, page_no, 'Y', 'Y', 'Grid'))
        ...
        if i_doc_ar:
            ins_stmt = """INSERT INTO ir_document_master(document_id, project_id, url_id, document_name, active_status, source_doc_info, source_type) values(%s, %s, %s, %s, %s, %s, %s)""" 
            m_cur.executemany(ins_stmt, i_doc_ar)
            m_conn.commit()
        if i_table_ar:
            stmt_t1 = """insert into norm_data_mgmt(norm_resid, project_id, url_id, docid, ref_resid, pageno, active_status, process_status, review_flag, source_table_info, source_type) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            m_cur.executemany(stmt_t1, i_table_ar)
            m_conn.commit() 
    
        if i_table_ar2:
            stmt_2 = """insert into data_mgmt(resid, project_id, url_id, docid, pageno, active_status , process_status, taxoname) values(%s, %s, %s, %s, %s, %s, %s, %s)"""
        ...
        print tmp_table_d
        return doc_ids_for_meta, tmp_table_d
    ...
    def get_company_info(self, sh_data_dct, project_id, s_type):
        ...
        for d_company_name, company_info_lst in  sh_data_dct.iteritems():
            ...
            data_builder_tt_wise, doc_table_html_cell_data, doc_ph_lst, order_class = company_info_lst 
            get_cid = c_d[company_name]
            #print get_cid, company_name, doc_ph_lst
            #dc_ph_dct = self.get_ph_dc_dct(company_name, project_id)  # no change
            dc_ph_dct = self.doc_ph_lst_dct(doc_ph_lst)
            #print doc_ph_lst
            #print dc_ph_dct
        
            #sys.exit()
            doc_ids_for_meta, tmp_table_d = self.working_with_d2_table_dict_data_doc_table_wise(company_name, project_id, get_cid, doc_table_html_cell_data, dc_ph_dct, s_type)
            table_d.update(tmp_table_d)
            self.add_meta_data_hra(company_name, doc_ids_for_meta, get_cid, project_id) # no change
