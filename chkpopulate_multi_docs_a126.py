...
def generate(company_id, lang_code='', given_doc_ids=None):
    project_id, url_id = company_id.split('_')
    all_doc_table_to_process = []
    norm_res_list = sObj.slt_normresids(project_id, url_id)
    for doc_tup in norm_res_list:
        doc_id, page_number, norm_table_id = map(lambda x:str(x), doc_tup)
        #if str(doc_id) != '77':continue
        #if norm_table_id not in ['1112']:continue
    ...
    error_list = []
    for ii, x in enumerate(all_doc_table_to_process):
        print ii+1, '/', total, x
        rf = generate_map_ds(company_id, x, lang_code)
        print '*'*100
        res.append(rf)
    company_id = org_company_id
    #res = pprocess.pmap(lambda x:generate_map_ds(company_id, all_doc_table_to_process[x], lang_code), range(0, len(all_doc_table_to_process)), 6)
    #######################################
...
def generate_map_ds(company_id, doc_tup, lang_code):
        ...
        if 1:
         for key, value_dict in celldata.items():
            ...
            for cell_id in cell_ids:
                ...
                tm_ar = []
                if section_type != 'GV' and lang_code:
                    #print 'orginal', section_type, all_txts
                    #ojson = sobj.send2(all_txts, '', 'es', 5)
                    ojson = sobj.send2(all_txts, 'TEXT', lang_code, 5)
                    #print 'cccc', ojson['o']
                    all_txts = ojson['o']
                    for txt in all_txts:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4:
        ...
        print doc_ids
        generate(company_id, lang_code, doc_ids)
    else:
        generate(company_id, lang_code)
