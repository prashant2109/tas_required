...
class  PC_mgmg(object):
    ...
    def insert_module_info(self):
        ...
        def rec_func(sub_lst, pk_info):
            ...
            for psk, c_dct in enumerate(sub_lst, 1):
                ...
                gactv       = c_dct['active']
                ci = (gc_n, gc_desc, gc_uk, gc_fi, str(gc_dc), p_info, clevel_id, select_flg, gactv)
                ci = map(str, ci)
                r_lst.append(ci)
        ...
        for pk, i_dct in enumerate(data_lst, 1):
            ...
            actv      = i_dct['active']
            di = (g_n, g_desc, g_uk, g_fi, str(g_dc), pk_info, level_id, select_flg, actv)
            di = map(str, di)
            res_lst.append(di)
        ...
        if res_lst:
            insert_stmt = """ INSERT INTO module_mgmt(module_name, description, module_key, fa_icon_info, doc_view, parent_key_info, level_id, user_select_flg, active_status) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)  """
    ...
    def user_save(self, ijson):
        ...
        read_qry = """ SELECT module_key, key_flag FROM project_module_key_map WHERE project_id='%s' """%(project_id) 
        m_cur.execute(read_qry)
        t_data = m_cur.fetchall()
        ...
        for key, flg in save_keys_dct.items():
            if key not in check_dct:
                insert_rows.append((project_id, key, flg))
            elif key in check_dct:
                get_chk_val = check_dct[key]
                if flg != get_chk_val:
                    update_rows.append((flg, key, project_id))
        if insert_rows:
            insert_stmt = """ INSERT INTO project_module_key_map(project_id, module_key, key_flag) VALUES(%s, %s, %s)  """
            m_cur.executemany(insert_stmt, insert_rows)
        if update_rows:
            update_stmt = """ UPDATE project_module_key_map SET key_flag=%s WHERE module_key=%s AND project_id=%s """ 
    ...
    def read_saved_info(self, ijson):
        ...
        rd_qry = """ SELECT module_key, key_flag FROM project_module_key_map WHERE project_id='%s'  """%(project_id)
    ...
    def read_all_modules(self, ijson):
        ...
        for row in t_data:
            ...
            dst = {'n':module_name, 'full_name':description, 'k':module_key, 'fa_icon':fa_icon_info, 'level_id':level_id, 'doc_view':bool(doc_view), 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if doc_view == 'EMPTY' and description != 'EMPTY' and fa_icon_info != 'EMPTY':
                dst = {'n':module_name, 'full_name':description, 'k':module_key, 'fa_icon':fa_icon_info, 'level_id':level_id, 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if description == 'EMPTY' and doc_view != 'EMPTY' and fa_icon_info != 'EMPTY':
                dst = {'n':module_name, 'k':module_key, 'fa_icon':fa_icon_info, 'level_id':level_id, 'doc_view':bool(doc_view), 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active} 

            if description != 'EMPTY' and doc_view != 'EMPTY' and fa_icon_info == 'EMPTY':
                dst = {'n':module_name, 'full_name':description, 'k':module_key, 'level_id':level_id, 'doc_view':bool(doc_view), 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if description == 'EMPTY'  and doc_view == 'EMPTY' and fa_icon_info != 'EMPTY':
                dst = {'n':module_name, 'k':module_key, 'fa_icon':fa_icon_info, 'level_id':level_id, 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if description == 'EMPTY'  and doc_view != 'EMPTY' and fa_icon_info == 'EMPTY':
                dst = {'n':module_name, 'k':module_key, 'level_id':level_id, 'doc_view':bool(doc_view), 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if description != 'EMPTY'  and doc_view == 'EMPTY' and fa_icon_info == 'EMPTY':
                dst = {'n':module_name, 'full_name':description, 'k':module_key, 'level_id':level_id,  'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}

            if description == 'EMPTY'  and doc_view == 'EMPTY' and fa_icon_info == 'EMPTY':    
                dst = {'n':module_name, 'k':module_key, 'level_id':level_id, 'usf':user_select_flg, 'submenu':[], 'pk':parent_key_info, 'active':active}
