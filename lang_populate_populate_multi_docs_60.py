...
import read_table_wise_user_info as rtw
r_Obj = rtw.user_info() 
...
def generate(company_id, given_doc_ids=None, lang=''):
    user_map = r_Obj.table_user_info(given_doc_ids)  
    project_id, url_id = company_id.split('_')
    all_doc_table_to_process = []
    norm_res_list = sObj.slt_normresids_20(project_id, url_id)
    #print len(norm_res_list)
    #sys.exit()
    for doc_tup in norm_res_list:
        doc_id, page_number, norm_table_id, gr_id = map(lambda x:str(x), doc_tup)
        get_user_name = user_map.get((str(doc_id), str(page_number), str(gr_id)), '')
        #if norm_table_id not in ['1964']:continue
        ktup = (doc_id, norm_table_id, page_number, gr_id, lang, get_user_name)
        ...
        if given_doc_ids:
            ...
            all_doc_table_to_process.append(ktup)
    #print '>>>>>', all_doc_table_to_process
    ...
    for doc_id, table_html_str_list in doc_table_html_dict.items():
        ...
        for (table_id, each_html_str) in table_html_str_list:
            if isinstance(each_html_str, unicode):
                each_html_str = each_html_str.encode('utf-8')
...
def generate_map_ds(company_id, doc_tup):
    ...
    project_id, url_id = company_id.split('_')
    doc_id, norm_table_id, page_no, gr_id, lang, g_user = doc_tup
    #if norm_table_id != '551':
    #    return [doc_id, norm_table_id, '', 0, 'ME', '']
    deal_id = '_'.join([project_id, url_id])
    if 1:#try:
        celldata_db_path = os.path.join(output_path, company_id, 'cell_data', doc_id, norm_table_id)
        ############################################
        celldata = lmdb_obj.read_all_from_lmdb(celldata_db_path)
        if 1:
         for key, value_dict in celldata.items():
            ...
            for cell_id in cell_ids:
                ...
                if not section_type:
                    section_type = ''
                all_txts = cell_info.get('text_lst', [])
                alt = cell_info.get('text_lst', [])
                if section_type != 'GV' and lang:
                    #print '>>>>', all_txts
                    ojson = sobj.send2(all_txts, 'TEXT', lang, 5)    
                    print ojson, all_txts
                    try:
                        all_txts = ojson['o']   
                    except:print doc_tup, sys.exit('EXIT')
                    #if "Agriculture " in ', '.join(all_txts) or "production plants"  in ', '.join(all_txts):
                    #    print 'sssss', ' '.join(alt), type(' '.join(alt))      
                    tm_ar = []
                    for txxt in all_txts:
                        if isinstance(txxt, unicode):
                            txxt = txxt.encode('utf-8')
                        tm_ar.append(txxt)
                    cell_info['text_lst'] = all_txts #tm_ar[:] 
                else:
                    cell_info['text_lst'] = all_txts[:]
        ...
        #sys.exit()
        if not celldata:
            print doc_id, norm_table_id, "No Cell Data"
        ################################################################
        row_col_dict2 = {}
        value_table_id_map = {}
        value_ref_map_all = {} 
        ...
        iusr, nusr = g_user, g_user#'test_user', 'dev'#= get_analyst_name(company_id, norm_table_id)
        #print '<<<<<<<<<', doc_tup
        html_str = display_table(data_ar, norm_table_id, doc_id, iusr, nusr, page_no, gr_id) 
        #print html_str
        #############################################################################
...
def display_table(data_ar, norm_table_id, docid, iusr, nusr, page_no, gr_id):
    if 0:
        page_no = ''
        if data_ar:
            ...
            for tmp_data in data_ar:
                for data in tmp_data:
                    ...
                    if page_no:break
                if page_no:break
    print 'HHHHH', [norm_table_id, docid, page_no, gr_id]
    html_str = '<div style="width: 100%;height: 4%;text-indent: 8px;background:#f3eeee;">'
    html_str += 'TABLE ID: '+ str(norm_table_id)+'  ||  ' + 'DOCUMENT ID: '+ str(docid) + '  ||  ' +'PAGE NO: ' + str(page_no) + '  ||  ' +'GRID ID: '+ gr_id +'  ||  '+'IAnalyst Name: '+iusr+'  ||  '+'NAnalyst Name: '+nusr 
    html_str += '</div>'
    ...
    for tmp_data in data_ar[:]:
        ...
        for data in tmp_data:
            ...
            print [norm_table_id, txt, section_type, type(txt)] 
            html_str += '<td section_type='+str(section_type)+' style="background:'+str(background_colr)+'" colspan='+str(colspan)+' rowspan='+str(rowspan)+'><span id="'+str(xml_ids)+'" table_id="'+str(norm_table_id)+'" id="'+str(xml_ids)+'">'+txt+'</span></td>'
        html_str += '</tr>' 
    html_str += '</table>'
    return html_str 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        doc_ids = doc_str.split('#')
        print doc_ids
        generate(company_id, doc_ids, lang)
    else:
        generate(company_id)
