...
class Delta(object):
    ...
    def gen_bboxs(self, company_id):
        ...
        total = len(norm_res_list)
        for i, doc_tup in enumerate(norm_res_list):
    ...
    def generate_map_ds(self, project_id, url_id, norm_table_id, doc_id):
        ...
        try:
            ...
            for key, value_dict in celldata.items():
                ...
                for cell_id in cell_ids:
                    cell_info = value_dict[cell_id]
                    print ' KEYYY', cell_id
                    print '  VALUEEE', cell_info, '<hr>'
                    #section_type = cell_info.get('section_type', '')
            #print celldata
            sys.exit()
        except Exception as e:
            print 'deal_id, norm_table_id>> :: NORM CELL DICT ERROR HHH',[deal_id, norm_table_id], e
            return
        #print celldata
        #sys.exit()
        ...
        try: 
            ...
            print 'deal_id, norm_table_id>> :: CSS ERROR',[deal_id, doc_id, norm_table_id], e
        #print [doc_id, norm_table_id, 'x4600_70']
        Xml_bb_dict = Xml_Cell_Obj.get_cell_bbox_data(project_id, url_id, doc_id, norm_table_id)
        #print Xml_bb_dict['x1000288_144#x2051_144'] 
        #sys.exit()
        #print Xml_bb_dict['x189_218'] 
        for x in 'x1000288_144#x2051_144'.split('#'):
            if x.strip() and x in Xml_bb_dict:
                print x
                print Xml_bb_dict[x] 
        sys.exit()
        '''
        for key, value_dict in celldata.items():
            ...
            for cell_id in cell_ids:
                ...
                #'isbold', 'indented_space'
                #print 'B',cell_info.get('isbold')
                #print 'I', cell_info.get('indented_space')
                #print '*'*100
        sys.exit() 
        '''
        celldata_db_path = os.path.join(self.output_path, deal_id, 'cell_data', doc_id, norm_table_id)
        ...
        os.system(cmd)
        if not os.path.exists(celldata_db_path):
            cmd = "mkdir -p %s" %(celldata_db_path)
            os.system(cmd)
        self.lmdb_obj.write_to_lmdb(celldata_db_path, celldata, celldata.keys())
        return
    ...
    def run(self, company_id):
        ...
        doc_page_dict = {}
        #ddfdict = {}
        #xlst = map(lambda x:('1_79', str(x)), range(1, 18))
        for doc_tup in norm_res_list:
            doc_id, page_number, norm_table_id = map(lambda x:str(x), doc_tup)
            #if doc_id != '34':continue
            if norm_table_id not in ['2']:continue
            #if norm_table_id not in ['62105']:continue
            #if norm_table_id not in ['18391']:continue
            if doc_id not in doc_page_dict:
                doc_page_dict[doc_id] = []
            doc_page_dict[doc_id].append(norm_table_id)
            #ddfdict[('1_79', doc_id)] = 1
        #print doc_page_dict
        #sys.exit()
        #sys.exit()
        #==========================================================
        ...
        for doc_id, table_list in doc_page_dict.items():
            ...
            for table_id in table_list:
                #print 'Cell data for : ', doc_id, table_id
                self.generate_map_ds(project_id, url_id, table_id, doc_id)
                #print 'Done Cell data for : ', doc_id, table_id  
