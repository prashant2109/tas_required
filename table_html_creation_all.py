...
class Delta(object):
    ...
    def display_table(self, data_ar, norm_table_id):
        ...
        html_str += 'TABLE ID: '+str(norm_table_id)
        html_str += '</div>'
        ...
        for tmp_data in data_ar[:]:
            ...
            for data in tmp_data:
                ...
                html_str += '<td section_type='+section_type+' style="background:'+background_colr+'" colspan='+str(colspan)+' rowspan='+str(rowspan)+'><span id="'+xml_ids+'" table_id="'+str(norm_table_id)+'" id="'+xml_ids+'">'+txt+'</span></td>'
            html_str += '</tr>' 
        html_str += '</table>' 
        return html_str 
    ...
    def get_clean_value(self, svalue):
        svalue = svalue.strip()
        flip_sign = ''
        if svalue.startswith('(')>0 and svalue.endswith(')')>0:
            flip_sign = '-'
        dot_count = 0
        new_string = ''
        for e in svalue:
    ...
    def generate_map_ds(self, project_id, url_id, norm_table_id, doc_id):
        deal_id = '_'.join([project_id, url_id])
        print 'deal_id, norm_table_id>> :: ',[deal_id, norm_table_id]
        celldata = self.Mobj.run_process(norm_table_id, deal_id)
        #try:
        #    celldata = self.Mobj.run_process(norm_table_id, deal_id)
        #except:
        #    print 'norm_table_id:', norm_table_id
        #    return 
    ...
    def run(self, company_id):
        ...
        self.doc_table_html_dict = {}
        done_doc_table_dict = {}
        doc_pairs_lst = self.get_doc_pairs(company_id)
        elms = {}
        for d in doc_pairs_lst:
            elms[d[0]] = 1
            elms[d[1]] = 1

        for k in elms.keys():
            self.doc_table_html_dict[k] = []
            all_table_ids1 = doc_page_dict.get(k, [])
            ar = map(lambda x:(int(x), x), all_table_ids1[:])
            ar.sort() 
            for j, selected_hyp in ar:
                self.generate_map_ds(project_id, url_id, selected_hyp, k)

        doc_html_path = os.path.join(lmdb_folder, 'Doc_Htmls')
        if not os.path.exists(doc_html_path):
            os.mkdir(doc_html_path)

        for doc_id, table_html_str_list in self.doc_table_html_dict.items():
            #print doc_id
            #print table_html_str_list
            #sys.exit() 
            doc_path    =  os.path.join(doc_html_path, str(doc_id))
            if not os.path.exists(doc_path):
                os.mkdir(doc_path)
            html_str = '<html><body>'
            for each_html_str in table_html_str_list:
...
if __name__ == "__main__":
    ...
    '''
    for company_id in ['1_62']:
        print "DELAL", company_id 
        #obj.get_data(company_id)     
        obj.run(company_id)
        #obj.generate_label_delta(company_id)
        print "DEAL END"
    ''' 
    company_id = '1_73'
    cl  = ['1_134', '1_77', '1_62'] #done
    cl  = ['75', '82', '34', '73', '51'] #done
    cl = ['44', '74', '58', '39', '71'] #done # 1_37 , 1_72pending
    cl = ['72', '78', '41', '46', '80', '65', '64']
    #old

    cl = ['77', '75', '82'] #done
    #'44' issue
    cl = ['74', '58', '39', '71', '72', '78', '41', '46', '80', '65', '64']
    for company_id in cl:
        company_id = '1_'+company_id
        print company_id, '*'*30
        obj.run(company_id)
    #print obj.find_other_values_gen('x1878_49', '1_62', '5')
    #print obj.find_other_values_gen('x1000529_50#x2908_50', '1_62', '5')
    #print obj.find_other_values_gen('x128_22', '1_62', '5')
