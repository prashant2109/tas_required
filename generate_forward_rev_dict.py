...
class Delta(object):
    ...
    def generate_table_item_map(self, company_id, company_name):
        ...
        doc_ref_text_info = {}
        doc_page_dict = {}
        slv_fname = os.path.join(self.output_path, company_id+'-filter.slv') 
        ddoc_tab_dict = self.lmdb_obj.read_from_shelve(slv_fname)
        for doc_tup in norm_res_list:
            doc_id, page_number, norm_table_id = map(lambda x:str(x), doc_tup)
            if doc_id not in ddoc_tab_dict:continue
            if norm_table_id not in ddoc_tab_dict[doc_id]:continue
        ...
        #print 'FINAL DOC-TAB PAIR', delta_table_match_list
        #sys.exit()
        ...
        print "doc-table-pair combintional ds created"
        #####################################################################################
        return 'done'
    ...
    def generate_label_delta_new(self, company_id, gdtup):
        lmdb_id, doc_pairs = gdtup
        lmdb_folder = os.path.join(self.output_path, company_id)
        lmdb_doc_table_pair_path = os.path.join(lmdb_folder, 'doc_table_pair')
        ...
        for doc_pair in doc_pairs:
            table_pairs = doc_table_pair_dict[doc_pair]
            ind1 += 1
            ind2 = -1
            for table_pair in table_pairs[:]:
                    ind2 += 1
                    print ' Complete: ', ind1 , ' / ', len(doc_pairs), ' : ', ind2, ' / ', len(table_pairs), ' lmdb_id: ', lmdb_id
        ...
        #############################################################################
        fname = os.path.join(lmdb_folder, 'doc_pair_forward_reverse_'+lmdb_id)   
        print fname 
        os.system('rm -rf '+fname)
    ...
    def get_doc_pairs(self, company_id, company_name): 
        deal_id = tuple(company_id.split('_'))
        slv_fname = os.path.join(self.output_path, company_id+'-filter.slv') 
        doc_tab_dict = self.lmdb_obj.read_from_shelve(slv_fname)
        doc_ph_info_dict = self.get_doc_ph_map(company_id) 
        all_phs = self.get_phs(company_name)[-1]
        dphs = {}
        ph_doc_info_dict = {}
        for doc_id, tables_dict in doc_tab_dict.items():
            if doc_id not in doc_ph_info_dict:continue
            ph = doc_ph_info_dict[doc_id]
            if ph not in ph_doc_info_dict:
                ph_doc_info_dict[ph] = []
            if doc_id not in ph_doc_info_dict[ph]:
                ph_doc_info_dict[ph].append(doc_id)
            
        sorted_ph_lst = [e for e in all_phs if e in ph_doc_info_dict]
        print 'sorted phs', sorted_ph_lst
        doc_pair_list = []
        ph_pair_list = []
        for i, ph in enumerate(sorted_ph_lst):
    ...
    def generate_label_delta_new_docids(self, company_id):
        lmdb_folder = os.path.join(self.output_path, company_id)
        lmdb_doc_table_pair_path = os.path.join(lmdb_folder, 'doc_table_pair')
        doc_table_pair_dict = self.lmdb_obj.read_all_from_lmdb(lmdb_doc_table_pair_path)
        new_allkey_dict = {}
        ...
        fwd_table_dict = {}
        rev_table_dict = {} 
        f = open('doc_pairs_txt/'+company_id+'.txt', 'w')
        for doc_pair in all_doc_pairs: 
            st = doc_pair[0] + ' ' + doc_pair[1] + '\n' 
            f.write(st)
        f.close()

 
 
    def create_delta_mutli(self, company_name, company_id):
        import math
        f = open('doc_pairs_txt/'+company_id+'.txt', 'r')
        flines = f.readlines()
        f.close()
        doc_pairs = map(lambda x:tuple(x.strip().split()), flines[:])
        n = float(len(doc_pairs))
        avg = n/50.0
        ...
        lmdb_grp_lst = []
        #avg = 2
        #print "No of Lmdb has to create ", avg
        #sys.exit()
        for lmdb_id in range(0, avg):
            new_doc_pairs = doc_pairs[min_bound:max_bound][:]
            lmdb_grp_lst.append((lmdb_id, new_doc_pairs))
            print [lmdb_id, min_bound, max_bound]
            min_bound += 50
            max_bound += 50
        res = pprocess.pmap(lambda x:self.generate_label_delta_new(company_id, lmdb_grp_lst[x]), range(0, len(lmdb_grp_lst)), 4)
        for elm in res:
            print 'anil', elm
        #print res
        #return 'done'
    ...
    def read_new_all_lmdb_data(self, lmdb_folder):
        FWDKEYS_dict = {}
        REVKEYS_dict = {}
        FWDRESULTS_dict = {}
        REVRESULTS_dict = {}
        ...
        with env.begin() as txn:
             ...
             for key, value in cursor:
                 ...
                 #print key  
                 if 'FWDKEYS:' in key:
                    FWDKEYS_dict[key] = value
                 if 'REVKEYS:' in key:
                    REVKEYS_dict[key] = value
                 if 'FWDRESULTS:' in key:
                    FWDRESULTS_dict[key] = value
                 if 'REVRESULTS:' in key:
                    REVRESULTS_dict[key] = value
        return FWDKEYS_dict, REVKEYS_dict, FWDRESULTS_dict, REVRESULTS_dict 


    def merge(self, company_id):
        lmdb_folder = os.path.join(self.output_path, company_id)
        all_results = {}
        results_dict = {}
        all_results2 = {}
        results_dict2 = {}
        for each in os.listdir(lmdb_folder):
            if 'doc_pair_forward_reverse_' in each:
                print 'Each: ', each 
                fname = os.path.join(lmdb_folder, each)
                ins_id = each.split('_')[-1]
                t_FWDKEYS_dict, t_REVKEYS_dict, t_FWDRESULTS_dict, t_REVRESULTS_dict = self.read_new_all_lmdb_data(fname)

                for k, vs in t_FWDKEYS_dict.items():
                    if k not in all_results:
                       all_results[k] = ''
                    all_results[k] += vs
                  
                for k, vs in t_REVKEYS_dict.items():
                    if k not in all_results2:
                       all_results2[k] = ''
                    all_results2[k] += vs
 
                for k, vs in t_FWDRESULTS_dict.items():
                    if k not in results_dict:
                       results_dict[k] = []
                    vs_ar = ast.literal_eval(vs)
                    results_dict[k] += vs_ar

                for k, vs in t_REVRESULTS_dict.items():
                    if k not in results_dict2:
                       results_dict2[k] = []
                    vs_ar = ast.literal_eval(vs)
                    results_dict2[k] += vs_ar

        if all_results:
            
            new_fname = os.path.join(lmdb_folder, 'doc_pair_forward_reverse')
            cmd = 'rm -rf %s' %(new_fname)
            os.system(cmd)
            print 'Writing to main db: ' , new_fname 
            env = lmdb.open(new_fname, map_size=20*1024*1024*1024)
            with env.begin(write=True) as txn:
                 for k, v in all_results.items():
                     txn.put(k, v) 
                 for k, v in all_results2.items():
    ...
    def remove_dir(self, company_id):
        lmdb_folder = os.path.join(self.output_path, company_id)
        for each in os.listdir(lmdb_folder):
            if 'doc_pair_forward_reverse_' in each:
                fname = os.path.join(lmdb_folder, each)
...
if __name__ == "__main__":
    company_name, company_id = sys.argv[1], sys.argv[2]
    obj = Delta()
    #print "start"
    obj.generate_table_item_map(company_id, company_name)
    obj.generate_label_delta_new_docids(company_id)
    #obj.create_delta_mutli(company_name, company_id)
    #obj.merge(company_id)
    #obj.remove_dir(company_id)
